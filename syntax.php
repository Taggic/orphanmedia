<?php
/**
 * OrphansMedia Plugin: Display orphan and missing media files
...
// ---------------------------------------------------------------
    ...
    function case1_good_output($c1_medialist,$img)
    {
        ...
        foreach($c1_medialist as $afile) {
            $i1++;                                                                                         
            $output.= '<td class="col0 centeralign"><a href="' . DOKU_URL . '/lib/plugins/orphanmedia/images/' . $img . '"><img src="' . DOKU_URL . '/lib/plugins/orphanmedia/images/' . $img . '" class="media" alt="orphan Media File" /></a></td>'
...
//    }
    ...
    */
    function connectTo($mode) {
        $this->Lexer->addSpecialPattern('~~ORPHANMEDIA~~',$mode,'plugin_orphanmedia');
...
// ---------------------------------------------------------------
  ...
  // 2. load all page files into array
     ...
     $delim = ".txt";
     clearstatcache();
     $listPageFiles = $this->list_files_in_array($conf['datadir'], $delim, $ns);
     $xArray =array();
    ...
    foreach($listPageFiles as &$page_filepath) {                            
         ...
         foreach($media_links_array as $media_link) {
          ...
          // ---------------------------------------------------------------  
             ...
             if((strlen($media_link)>1) && strpos($media_link, "://")<1 ) {
...
// ---------------------------------------------------------------------------//
      ...
      foreach($c1_medialist as $media_file) {
         ...
         foreach($listPageFiles as $x_link) {                  
              ...
              if(strpos($x_link,basename($media_file))>1) {
                  $checkFlag = TRUE;
                  $a_counter++; 
         ...
         if($checkFlag === FALSE) {
            ...
            //cut everything before media/
            $y_pos=strpos($media_file, "media");
            $t1 = substr($media_file, $y_pos);
            $len_m_filename = strlen($m_filename);
            $m_path = substr($t1, 0, -$len_m_filename);            
            
            $orphan_Media_Files[] = $m_path . "</td><td>" . $m_filename;
            $b_counter++;
         }
      }
      ...
      foreach($listPageFiles as $x_link) {
          ...
          foreach ($x_array as $tst) {
              ...
              if($checkFlag === FALSE) {
                  ...
                  //cut everything before media/ from link
...
//--------------------------------------------------------------------------------------------------------   
        ...
        $output.= '<h2><a name="missing_media" id="missing_media">Missing Media Files</a></h2><div class="level1">';
        ...
        $output.= '<table class="inline"><tr><th  class="col0 centeralign">i</th><th  class="col1 centeralign">#</th><th  class="col2 centeralign"> Media path </th><th  class="col3 centeralign"> Media files </th></tr>';   
        $img = "nok.png";
            ...
            while (FALSE !== ($sub = readdir($handler))) { 
                if ($sub !== "." && $sub !== "..") { 
                    ...
                    elseif(is_dir($dir."/".$sub)) { 
                        $listDir[$sub] = $this->list_files_in_array($dir."/".$sub, $delim);
                        //if(DEBUG) echo sprintf("<p><b>%s</b></p>\n", $dir."/".$sub);
                    } 
                } 
            }    
